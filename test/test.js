...
global.assertAlmostEqual = function (a, b, precision, message) {
    ...
    wrapAssertion(
        () => assertEqual(roundA, roundB, message),
        roundA,
        roundB,
        "~"
    );
...
["test.Turndown", "test.TextRank", "test.Ellipsis.dom"].forEach(
    async (reference) => {
        await import(
            join(import.meta.dirname, reference.replace(/(\.js)?$/i, ".js"))
        );
    }
);
